...
function FormFiscalAsignarAccion({ primeraDiligenciaSeleccionada, closeModalAccionPrimeraDiligencia, getAllPrimerasDiligencias }) {
    const navigate = useNavigate();
    const { url } = useStateContext();
    const endpointRealizarAccion = `${url}/api/primerasDiligencias/inhabilita-acepta-cargo/`;
    ...
    const [estadoDocumentoSelect, setEstadoDocumentoSelect] = useState(false);
            ...
            console.log("seleccionaste este pedeefe: ", documentoPPDD)

    const handleChangeSelectAccion = (e) => {
        setEstadoPrimeraDiligencia(e.target.value);
        const valor = e.target.value;

        setEstadoDocumentoSelect(true); // mostrar input
        switch (valor) {
    ...
    const handleRealizarAccion = async () => {
        ...
        try {
            ...
            if (response.data && response.data.message) {
                ...
                Swal.fire({
                    title: 'Exito',
                    text: 'Se ha realizado una acción en la primera diligencia exitosamente.',
                    icon: 'success',
                    confirmButtonText: 'Aceptar',
                    confirmButtonColor: '#3085d6'
                }).then(() => {
                    getAllPrimerasDiligencias();
                    closeModalAccionPrimeraDiligencia();
                        ...
                        <div className="mt-4 mb-4">
                            <div className="flex items-center justify-between gap-2">
                                <div className="w-full">
                                    ...
                                    <select
                                        className="w-full p-1 border border-gray-400 rounded-lg shadow-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                        defaultValue=""
                                        onChange={handleChangeSelectAccion}
                                    >
                                        <option value="" disabled>Seleccione una opción</option>
                                        {primeraDiligenciaSeleccionada.estado_primera_diligencia === 'Pendiente Aceptación de cargo' && (
                                            <>
                                                <option value="Aceptación de Cargo">Aceptación de Cargo</option>
                                                <option value="Inhabilita">Inhabilita</option>
                                            </>
                                        )}
                                        {primeraDiligenciaSeleccionada.estado_primera_diligencia === 'Aceptación de Cargo' && (
                                            <>
                                                <option value="Prórroga">Prórroga</option>
                                                <option value="Oficio informe">Oficio informe</option>
                                            </>
                                        )}
                                        {primeraDiligenciaSeleccionada.estado_primera_diligencia === 'Dispone notificación' && (
                                    ...
                                    {estadoDocumentoSelect ? (
                                        <div className="w-full">
                                            ...
                                            <input
                                                ...
                                                className="w-full p-1 border border-gray-400 bg-white rounded-lg shadow-sm text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                            />
                                        </div>
                                    ) : (
                                        <div className="w-full">
                                            ...
                                            <input
                                                type="text"
                                                className="w-full p-1 border border-gray-400 placeholder:text-gray-600 bg-white rounded-lg shadow-sm text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                                placeholder="Seleccione una acción para carga el documento"
                                                readOnly
                                            />
                                        </div>
                                    )}
