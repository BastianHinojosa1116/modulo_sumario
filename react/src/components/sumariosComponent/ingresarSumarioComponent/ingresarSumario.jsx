...
function IndexDisponeSumario() {

    //endpoint para utilizar en los botones
    const navigate = useNavigate();
    const { url } = useStateContext(); // Cambiado aquí
    // const url = 'http://localhost:8000';
    const obtenerFiscal = `${url}/api/sumario/obtenerFiscal`;
    const endpointDisponerSumario = `${url}/api/disponerSumario`;
    ...
    const [nroDocumentoOriginaSumario, setNroDocumentoOriginaSumario] = useState('');
    const [fechaDocumentoDisponeSumario, setFechaDocumentoDisponeSumario] = useState('');
    //fiscal seccion
    ...
    const resetForm = () => {
        ...
        setIsVisible(false);
        setInvolucrado([]);
        setInvolucradosIngresados('');
        setPersonasAgregadas([]);
        setUserData(null);
        setBuscarRut('');
        setDocumentoPPDD(null);
        setNroDocumentoOriginaSumario(null);
    };
    ...
    const handleSubmitFormularioPPDD = async () => {
        ...
        const fechaOrdenDocumentaOriginaSumario = convertirFecha(fechaDocumentoDisponeSumario);
        ...
        formData.append('fecha_dispone_sumario', fechaDisponePPDDFormateada);
        formData.append('documento_sumario', documentoPPDD);
        formData.append('plazo', plazoFormateada);
        formData.append('nro_documento_dispone_sumario', nroDocumentoOriginaSumario);
        formData.append('fecha_documento_dispone_sumario', fechaOrdenDocumentaOriginaSumario);
        ...
        try {
            ...
            const response = await axios.post(endpointDisponerSumario, formData, {
                headers: {
            ...
            if (response.data && response.data.message) {
                ...
                Swal.fire({
                    ...
                    navigate('/sumarios/ingresarFalta'); // si querés redirigir también
    ...
    return (
        <div className='pl-2 pr-2 mb-16'>
            <div className="max-w-full  mt-4 p-6 bg-white shadow-lg border border-gray-300 rounded-lg">
                <h2 className="text-2xl  font-bold mb-6 text-center text-gray-700">Formulario Ingresa Sumario</h2>
                <div className="space-y-4">
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                        ...
                        {/* Fecha dispone PP.DD */}
                        <div>
                            <label className="block text-gray-700 font-medium mb-2">Fecha dispone Sumario:</label>
                            <input
                                type="date"
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                        {/* Cargar documento PP.DD */}
                        <div>
                            <label className="block text-gray-700 font-medium mb-2">Nro/Orden documento que origina O. Sumario</label>
                            <input
                                type="number"                                 
                                value={nroDocumentoOriginaSumario}          
                                onChange={(e) => setNroDocumentoOriginaSumario(Number(e.target.value))}              
                                className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                            />
                        </div>
                        {/* Plazo calendario */}
                        <div>
                            <label className="block text-gray-700 font-medium mb-2">Fecha documento dispone sumario:</label>
                            <input
                                type="date"
                                value={fechaDocumentoDisponeSumario}
                                onChange={(e) => setFechaDocumentoDisponeSumario(e.target.value)}
                                max={new Date().toISOString().split("T")[0]} // Restringe fechas futuras
                                className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                onKeyDown={(e) => e.preventDefault()} // Evita que el usuario escriba con el teclado
                            />
                    ...
                    <div>
                        {/* Motivo de la PP.DD */}
                        <label className="block text-gray-700 font-medium mb-2">Motivo del sumario:</label>
...
export default IndexDisponeSumario
