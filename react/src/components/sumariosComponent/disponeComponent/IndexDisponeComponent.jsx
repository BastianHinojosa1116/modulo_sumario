...
function IndexDisponeComponent() {
  ...
  const redirigirFormFiscal = (sumario) => {
  if (!sumario?.id) {
    console.warn("El sumario no tiene ID válido:", sumario);
    return;
  }

  navigate(`/sumarios/fiscalTramitar/${sumario.id}`, {
    state: {
  ...
  const getAllSumarios = async () => {
    try {
      ...
      const response = await axios.get(`${url}/api/sumarios/fiscales`, {
        params: {
          page: currentPage,
          search: searchTerm,
          rut: rut // 
      ...
      console.error('❌ Error al obtener los sumarios:', error.response?.data || error.message);
      Alertas.error('No se pudieron cargar los sumarios.');
    }
  };
                  ...
                  <tr key={sumario.id} className="hover:bg-slate-200 transition">
                      ...
                      <ul className="flex flex-col gap-1">
                        {sumario.involucrados_sumarios?.map((relacion) => {
                          ...
                          return (
                            ...
                            >
                              <span className="font-semibold">NOMBRE:</span> {inv?.primer_nombre}{' '}
                              {inv?.apellido_paterno} –{' '}
                    ...
                    <td className="px-4 py-2 text-center">
                      <div className="flex flex-col gap-2">
                        <button
                          onClick={() => redirigirFormFiscal(sumario)}
                          className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md text-sm shadow-md transition"
                        >
                          Realizar Acción
                        </button>
...
export default IndexDisponeComponent;
