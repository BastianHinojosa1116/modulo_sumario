...
import ModalInfoUsuarios from '../resolverFaltaDisciplinariaComponent/ModalInfoUsuarios'; // Importa el componente ModalInfoUsuarios
...
function AplicarSancion() {
    ...
    const [openModalInformacionUsuario, setOpenModalInformacionUsuario] = useState(false);
    ...
    const handleInformacionUsuarioClick = (involucrados) => {
        setUsuario(involucrados);
        setOpenModalInformacionUsuario(true);
    };

    const closeModalInformacionUsuario = () => {
        setOpenModalInformacionUsuario(false);
        getFaltaById();
    };
    ...
    return (
        <div className="mx-auto p-4 md:p-10 rounded-xl bg-white lg:ml-16 lg:mr-16 mb-24 border mt-8 border-gray-300">
            <h1 className="text-2xl md:text-4xl text-gray-700 font-bold text-center mb-6 md:border-b-2 md:border-gray-700 lg:ml-16 lg:mr-16">Resolver falta disciplinaria</h1>
            <form className="">
                                ...
                                {involucrado.length > 0 ? (
                                    involucrado.map((involucrados, index) => (
                                        <tr key={index} className="border-b transition-colors duration-500 hover:bg-gray-200">
                                            <td className="py-2 px-4 flex justify-center">{involucrados.name} </td>
                                            ...
                                            <td className="py-2 px-4">
                                                {involucrados.estado_sancion !== "Sanción aplicada" ? (
                                                    <>
                                                        <div className="flex justify-center space-x-2">
                                                            {/* agregar botones */}
                                                            <img
                                                                onClick={() => handleInformacionUsuarioClick(involucrados)}
                                                                src="/src/assets/iconos/informacionn.png"
                                                                alt="Ícono de información"
                                                                className="h-8 w-8   transform transition duration-300 hover:scale-110 "
                                                            />
                                                            <strong className='text-red-600'>Sanción Resuelta</strong>
                                                        </div>
                                                    </>
                                                ) : (
                                                    <div className="flex justify-center space-x-2 mr-2">
                                                        ...
                                                        />
                                                        <Link
                                                            to={`/procesosDisciplinarios/resolverFalta/aplicarSancion/${falta.id}/involucrado/${involucrados.id}`}
            ...
            <ModalInfoUsuarios openModalInformacionUsuario={openModalInformacionUsuario} closeModalInformacionUsuario={closeModalInformacionUsuario} usuario={usuario} />
            <ToastContainer />
        </div>
    )
}

export default AplicarSancion
