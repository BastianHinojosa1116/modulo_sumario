...
import ModalAplicarSancion from '../resolverFaltaDisciplinariaComponent/ModalAplicarSancion'; // Importa el componente Modalaplicarsancion
import ModalNoAplicarSancion from '../resolverFaltaDisciplinariaComponent/ModalNoAplicarSancion'; // Importa el componente Modalnoaplicarsancion
import ModalResuelveSancion from '../resolverFaltaDisciplinariaComponent/ModalResuelve'; // Importa el componente Modalnoaplicarsancion
import { useStateContext } from '../../../contexts/ContextProvider'; // Cambiado aquí

function AplicarSancionInvolucrado() {
    ...
    const [openModalResuelve, setOpenModalResuelve] = useState(false);
    ...
    const [verUltimaSancionInvolucrado, setVerUltimaSancionInvolucrado] = useState([]);
    const [error, setError] = useState(null);
    const [idUltimaSancion, setIdUltimaSancion] = useState('');
    ...
    // Abrir modal aplicar sancion
    const closeModalResuelve = () => {
        setOpenModalResuelve(false);
    };
    const handleResuelveClick = () => {
        setOpenModalResuelve(true);
    };
    ...
    const getInvolugradoById = async () => {
        try {
            ...
            console.log('caida:', response.data.involucrado.sanciones);
    ...
    const fetchUltimaSancion = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                setIdUltimaSancion(response.data.ultima_sancion.estado_sancion);
                //setSanciones(response.data.ultima_sancion.sancion);

                console.log('caida 2:', response.data.ultima_sancion.estado_sancion);
            } else {
    ...
    console.log('id ultima sancion:', idUltimaSancion);
    ...
    return (
        <div className="mx-auto p-4 md:p-10 rounded-xl bg-white lg:ml-16 lg:mr-16 mb-24 border mt-8 border-gray-300">
            ...
            <div className='overflow-hidden mt-8 mb-8 bg-gray-100 rounded-lg shadow-md p-6 lg:ml-16 lg:mr-16 border border-gray-300'>
                ...
                <div className="flex justify-center">
                    {idUltimaSancion && (
                        ...
                        focus:ring-2 focus:ring-primary-400 transition duration-500'
                            type="button"
                        >
                            Notificar Sanción
                        </button>
                    ...
                    {!idUltimaSancion && (
                        <button
                            onClick={() => handleResuelveClick()}
                            className='bg-green-700 mr-2 hover:bg-green-500 text-white px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-green-400 transition duration-500'
                            type="button"
                        >
                            Resolver
                        </button>
                                ...
                                <div key={sancion.id} className='ml-4 mr-4 mt-4 mb-4'>
                                    <div className="p-2 ml-2 mt-2 mr-2 rounded-t-lg text-center border bg-gray-600 border-gray-400">
                                        <p className="font-bold text-md text-white">
                                            ...
                                            <span
                                                className={`ml-2 inline-block w-3 h-3 rounded-full ${sancion.aplica_sancion === 'Sanción aplicada' ? 'bg-red-500' :
                                                    sancion.aplica_sancion === 'Sanción no aplicada' ? 'bg-green-500' :
                                                        'bg-yellow-500'
                                                    }`}
                                            ></span>
                                        </p>
                                    ...
                                    <div className="p-5 flex justify-between -lg ml-2 rounded-b-lg mr-2 border bg-gray-100 border-gray-400">
                                        <div className="w-1/2 ml-2">
                                            ...
                                            <p className='p-1'><strong>Medida adoptada:</strong> {sancion.sancion}</p>
                                            {sancion.dias_arresto !== "0" && (
                                                <p className='p-1'><strong>Días de arresto:</strong> {sancion.dias_arresto}</p>
                                            )}
            ...
            <ModalAplicarSancion sanciones={sanciones} openModalAplicarSancion={openModalAplicarSancion} closeModalAplicarSancion={closeModalAplicarSancion} id={id} />
            ...
            <ModalResuelveSancion openModalResuelve={openModalResuelve} id={id} closeModalResuelve={closeModalResuelve} />
        </div>
    )
}

export default AplicarSancionInvolucrado
