...
function IndexBandejaResolucionesComponent() {
    //declaracion de variables
    const [faltas, setFaltas] = useState([]);
    const { url } = useStateContext(); // contexto para traer la url del sitio
    const endpoint = `${url}/api/bandejaResoluciones`;
                                    ...
                                    {faltas.length > 0 ? (
                                        faltas.map((falta) => (
                                            <tr key={falta.id} className="border-b transition-colors duration-500 hover:bg-gray-200">
                                                ...
                                                <td className="py-2 px-4 text-red-600">{falta.estado_proceso}</td>
                                                <td className="py-2 px-4">
                                                    <div className="flex justify-center space-x-2">
                                                        {/* agregar botones  */}
                                                        <Link
                                                            to={`/procesosDisciplinarios/bandejaResoluciones/${falta.id}`}
...
export default IndexBandejaResolucionesComponent
