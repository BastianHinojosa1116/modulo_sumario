...
function FormReapertura({ closeModalReapertura, id }) {
    const [fechaReapertura, setFechaReapertura] = useState('');
    const [documentoFundamentoReapertura, setDocumentoFundamentoReapertura] = useState(null);

    const navigate = useNavigate();
    const { url } = useStateContext(); // Cambiado aquí
    const endpoint = `${url}/api/procesosDisciplinarios/bandejaResoluciones/reapertura/`;
    ...
    const handleChangeDocumentoFundamentoReapertura = (e) => {
        setDocumentoFundamentoReapertura(e.target.files[0]);
        console.log(documentoFundamentoReapertura);
    };

    const handleChangeFechaReapertura = (e) => {
        setFechaReapertura(e.target.value);
        console.log('Fecha seleccionada:', e.target.value);
    };

    const handleReapertura = async (e) => {
        ...
        formData.append('documento_fundamento_reapertura', documentoFundamentoReapertura);
        ...
        try {
            ...
            if (response.data && response.data.message) {
                ...
                Swal.fire({
                    ...
                    text: 'Se ha realizado la reapertura con exito.',
                    icon: 'success',
            ...
            if (error.response && error.response.data.errors) {
                ...
                Swal.fire({
                    title: 'Error',
                    text: 'Hubo un error al intentar realizar la reapertura de la sanción. Inténtalo de nuevo.',
                    icon: 'error',
                    confirmButtonText: 'Aceptar',
                    confirmButtonColor: '#3085d6'
                });
    ...
    const handleButtonClickReapertura = () => {
        Swal.fire({
            title: '¿Estás seguro?',
            text: 'Al confirmar se realizará la reapertura del caso.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                handleReapertura(); // Solo ejecuta si se confirma
            }
        });
    };
        ...
        <div>
            <div className='ml-6 mr-6 mt-6 mb-6 p-8 bg-white rounded-lg border-2 border-gray-300'>
                <div className="border-b-2 border-gray-400 pb-2 ml-6 mr-6 mb-3 ">
                    <h4 className="text-2xl text-gray-700 font-bold text-center">Reapertura del caso</h4>
                </div>
                <p className='font-serif text-center text-gray-500'>Debe cargar el documento que fundamente la razón de la reapertura del caso. </p>
                ...
                <div className='flex justify-center items-center p-4'>
                    ...
                    <input
                        ...
                        type="file"
                        id="reapertura"
                        onChange={handleChangeDocumentoFundamentoReapertura}
                    />
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        onChange={handleChangeFechaReapertura}
            ...
            <div className='overflow-hidden mt-4 mb-8 bg-white rounded-lg shadow-md p-6 ml-6 mr-6 border border-gray-300'>
                ...
                focus:ring-2 focus:ring-primary-600 transition duration-500'
                        type="button"
                        onClick={handleButtonClickReapertura}
                    >
                        Enviar
                    </button>
